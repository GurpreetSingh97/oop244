...
namespace sict {
	...
	std::ostream& NonPerishable::write(std::ostream& os, bool linear) const { //change it
		if (err.isClear() == true) { // problem is here
			...
			// implement straight line display here
				...
				os << "Quantity Needed: " << required_quantity;
			}
		}
		else {
			os << err.message();
	...
	std::istream& NonPerishable::read(std::istream& is) { // change it
		...
		// same brackets so that if the former cin crash the latter wont work
		if (!is.fail()) {
		...
		if (!is.fail()) {
			strcpy(this->sku, temp_sku);
			this->name(temp_item_name);
			strcpy(this->unit, temp_unit);
			this->price = temp_price;
			this->current_quantity = temp_current_quantity;
			this->required_quantity = temp_required_quantity;
			this-> taxable = temp_taxable;
			this->err.clear();
